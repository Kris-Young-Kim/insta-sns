 ...
 * @description 팔로잉 목록 조회 API Route
 * 
 * GET: 특정 사용자의 팔로잉 목록 조회
 */
...
export async function GET(
  ...
  try {
    ...
    // 2. 팔로잉 목록 조회 (targetUserId가 팔로우하는 사용자들)
    ...
    if (followingError) {
      console.error("팔로잉 조회 에러:", followingError);
    ...
    console.log("팔로잉 목록 조회 성공:", { count: followingWithFollowStatus.length });
    ...
    console.error("팔로잉 목록 API 에러:", error);
    return NextResponse.json(
      { error: "서버 오류가 발생했습니다." },
      { status: 500 }
    );
